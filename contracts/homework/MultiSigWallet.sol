...
contract MultiSigWallet {
  ...
  receive() external payable {
   ...
   modifier notExecuted(uint256 _txId){
    require(!transactions[_txId].exected,"tx already exected");
    _;
   }
   ...
   function approv(uint256 _txId) external onlyOwner txExists(_txId) notExecuted(_txId)
   {
   ...
   {
     require(getApprovalCount(_txId) >= _required, "approval < required");
     Transaction storage transaction = transactions[_txId];
     transactions.exected = true;
